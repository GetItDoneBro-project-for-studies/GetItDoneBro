    ...
    : ITokenService, IDisposable
{
    ...
    private async Task<AccessTokenModel> FetchTokenAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(exception: ex, message: "Error fetching OAuth token from {TokenEndpoint}", tokenEndpoint);
            throw new IntegrityException($"Nieoczekiwany błąd podczas pobierania tokenu OAuth z {tokenEndpoint}", ex);
        }
    }
